...
- **사용설명의 ‹ ›는 단계를 넘긴 뒤 그 단계가 가리키는 곳이 가려져 있으면 스크롤한다**(`revealCurrentSpot`). 폰이 730px이라 844px 화면에 다 들어오지 않고 강조 위치가 단계마다 흩어지기 때문이다. **이미 보이면 움직이지 않는다** — 이유 없이 화면이 흔들린다. 자동재생 중에는 부르지 않는다(읽는 중 페이지가 저 혼자 움직이면 안 된다). 대상 우선순위는 모달 → `tap` 대상 → 화면(`scr-*`) 순.
