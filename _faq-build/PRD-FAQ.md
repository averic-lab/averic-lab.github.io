...
- **FAQ 본문 카피 미작성.** 4개 주제의 질문·답변 한국어 원문이 아직 없다. 이 카피는 앱 번역 파일에 없으므로 **20개 언어 번역이 별도로 필요**하다(추출 대상이 아님). 카피 확정 후 번역 방식을 정한다.
- 앱 문구(`.tr` 키)와 FAQ 설명문은 **출처가 다르다** — 전자는 추출, 후자는 직접 작성. 혼동하지 말 것.
