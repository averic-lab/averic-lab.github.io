...
- ⚠️ **좁은 화면(≤920px)에서는 라벨이 `접기`로 바뀌지 않는다.** 이 버튼은 거기서 "화면으로 데려가 줘" 하나만 뜻하고, 이미 펼쳐진 상태에서 다시 눌러도 접지 않고 목업으로 다시 스크롤한다. 문맥 바로 설명에 돌아온 뒤 **"그 화면 다시 보고 싶다"가 가장 흔한 다음 행동**인데, 접기 상태였을 때는 [접기] → [보기] 두 번을 눌러야 했다(사용자 지적, 2026-08-09). 펼침 해제는 다른 주제를 열 때 `collapseAll`이 알아서 한다. 라벨 판정은 `syncShowMeLabels()` 한 곳에 모아 두고 화면폭이 바뀔 때 다시 맞춘다 — 좁은 화면에서 펼친 채 넓히면 라벨과 동작이 어긋나기 때문이다.
