...
- 이 버튼 하나가 **둘을 동시에** 한다: 목업에서 해당 경고 강조 + 그 경고를 탭했을 때 뜨는 다이얼로그 펼치기.
- ⚠️ **이 버튼은 접기가 되지 않는다(라벨 고정).** 이미 펼쳐진 상태에서 다시 눌러도 접지 않고 위치를 다시 알린다(좁은 화면에서는 목업으로 다시 스크롤). 접기를 없앤 이유는 폭마다 다르다 — **좁은 화면**: 문맥 바로 설명에 돌아온 뒤 "그 화면 다시 보고 싶다"가 가장 흔한 다음 행동인데 [접기] → [보기] 두 번을 눌러야 했다. **넓은 화면**: 목업이 sticky로 옆에 붙어 있어 접을 이유가 없고, 전체 화면으로 되돌리는 일은 상단 **[전체 화면] 칩**이 이미 한다(사용자 지적, 2026-08-09). 펼쳐진 상태는 라벨이 아니라 **버튼이 teal로 채워지는 것**(`.show-me[aria-expanded="true"]`)으로 드러나며, 펼침 해제는 다른 주제를 열 때 `collapseAll`이 한다. 이에 따라 카피 키 `mockup_hint_close`("접기")와 `data-close` 속성은 20개 언어에서 **제거**됐다 — 되살리려면 위 두 이유를 먼저 반박할 것.
...
- **배경은 반투명 다크(`rgba(26,28,28,.72)` + `blur(18px)`)다.** 뒤가 비쳐 "떠 있는 판"으로 읽히면서도, 가장 밝은 배경(흰 카드) 위에서 흰 글자 대비가 약 **6.9:1**로 AA(4.5:1)를 넉넉히 넘는다. **0.6까지 내리면 4.6:1로 경계에 걸리므로 더 내리지 말 것.** 투명·모션을 원치 않는 환경(`prefers-reduced-transparency` / `prefers-reduced-motion`)에서는 불투명 `var(--ink)`로 떨어진다.
- **사용설명은 단계가 바뀌면 그 단계가 가리키는 곳을 화면 안으로 데려온다**(`revealCurrentSpot`). 폰이 730px이라 844px 화면에 다 들어오지 않고 강조 위치가 단계마다 흩어진다 — 실측(390×844, 스크롤 0)으로 **6~9단계 대상이 전부 화면 밖**이라 자동재생이 도는 동안 바의 숫자만 올라가고 정작 무엇이 강조되는지 보이지 않았다(사용자 지적, 2026-08-09). 그래서 **‹ › 뿐 아니라 자동재생 중에도 따라간다.** 세 가지 안전장치가 붙는다: ⓐ **가려져 있을 때만** 옮긴다(이미 보이는데 스크롤하면 이유 없이 흔들린다 — 실측상 9단계 동안 스크롤은 두 번만 일어난다), ⓑ 사용자가 **손으로 스크롤한 순간 따라가기를 끈다**(`autoFollow` 래치, `wheel`·`touchmove`만 듣는다 — `scroll`을 들으면 우리가 부른 부드러운 스크롤에도 스스로 꺼진다), ⓒ ‹ ›·단계 탭·다시보기처럼 "따라가겠다"는 조작을 하면 다시 켜진다. 대상 우선순위는 모달 → `tap` 대상 → 화면(`scr-*`) 순.
- ⚠️ **따라가는 대상은 목업의 강조 지점이지 단계 "설명"이 아니다.** 설명으로 스크롤하면 폰이 화면 밖으로 밀려 애니메이션을 못 보게 된다(둘을 한 화면에 넣을 수 없다는 위 전제 그대로). 설명 전문을 읽는 경로는 **바의 제목 탭** 하나다.
- ⚠️ **사용설명 바에는 FAQ 같은 텍스트 알약("설명 보기")을 넣을 수 없다.** 실측(2026-08-09): 390px에서 `.ctxmain` 290px − 번호 배지 41 − 여백 16 = 제목에 **215px**. 여기에 알약을 넣으면 제목 자리가 **85px**로 줄고, 320px에서는 **15px**로 사실상 사라진다. 알약 후보 폭은 이탈리아어 148 · 스페인어 137 · 프랑스어 132px다(13px/700 기준). FAQ 바는 ‹ ›(합 100px)가 없어서 가능했던 것이고, 사용설명은 그 자리를 이동 버튼이 쓴다. 바의 핵심 가치인 **단계 제목을 희생하게 되므로 아이콘(18px)을 유지한다.**
