...
- ⚠️ **grid/flex 안에서는 `keep-all` 이 가로 넘침을 만든다.** 한국어는 띄어쓰기가 있어 안전하지만
  **일본어·중국어는 공백이 없어 절 하나가 통째로 끊기지 않는 "단어"** 가 되고, 그 `min-content` 가
  `1fr`(= `minmax(auto,1fr)`) 열을 밀어 올린다. 2026-09-03 「지킨 네 가지」에서 ja 380px 카드가
  402px 로 부풀어 페이지에 가로 스크롤이 생겼다. → `minmax(0,1fr)` + `min-width: 0`.
  `overflow-wrap: break-word` 는 **해결책이 아니다**(min-content 계산을 바꾸지 않는다).
  기존 `.step h3/p` 에 keep-all 이 없는 것도 같은 이유로 보인다. 상세는 「지킨 네 가지」 절.
...
## 「지킨 네 가지」 섹션 — 타사와의 차이를 부정형으로 (2026-09-03)
...
- **`보호자`는 사이트의 기능 어투를 쓴다** — `flow_lbl_guardian` 기준
  ...
  ⚠️ `step1_p` 계열의 감성 어투("your loved ones")와 **다르다**. 이 섹션은 제품 됨됨이를
  말하는 자리라 기능 어투가 맞다.
...
2. **`.step`/`.steps` 를 재사용하지 않는다.** 그쪽은 `grid repeat(2,1fr)` 로 2장에 맞춰져 있고
   `.step` 은 `opacity:0` 이라, 물려받으면 4장 배치가 깨지고 카드가 통째로 안 보인다.
   그래서 접두사가 `.made*` 다.
