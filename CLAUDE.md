...
- **알림 카드가 좌·우로 번갈아** 튀어나온다(`.np1`~`.np4`). 좌우 교차가 목적이므로 `left` 값을 한쪽으로 몰지 말 것.
- 떠난 자리는 히어로와 **같은 규칙**으로 반투명 유리(`.nlist .ncard::after`)가 덮는다. 정보 카드만 튀어나오지 않아 유리를 빼 둔다(`.nc-info::after { content: none }`).
