...
### ③ 자리의 숏폼 영상 (2026-09-19, 한국어만)

`i18n/build.py`의 `SHORTS`에 있는 언어는 **③ 문장 대신 숏폼 영상 두 편**이 들어간다(`{{DAWN_TAIL}}`).
③ 문장은 사라진 게 아니라 **영상의 공통 엔딩으로 옮겨 갔다** — 두 편 모두 같은 문장과 노을 배경으로 끝난다.
`SHORTS`에 없는 언어는 지금처럼 ③ 문장을 보여 준다. 현재 `ko`만 등록돼 있다.

- 영상 파일: `media/shorts/<언어>/{a,b}.mp4` + 포스터 `.jpg`. 검토 페이지 `preview/shorts/`도 같은 파일을 쓴다.
...
- ⚠️ **언어를 추가할 때는 영상 파일부터 올린다.** `SHORTS`에만 넣으면 그 언어 홈에 깨진 영상이 뜬다.
- ⚠️ 영상은 다시 렌더할 때마다 git 기록에 새로 쌓인다(편당 약 1.4MB). 20개 언어 × 2편을 자주 고치면
  저장소가 빠르게 커지므로 **한국어판을 확정한 뒤 다국어를 한 번에** 만든다.
