...
- ⚠️ **grid/flex 안에서는 `keep-all` 이 가로 넘침을 만든다.** 한국어는 띄어쓰기가 있어 안전하지만
  **일본어·중국어는 공백이 없어 절 하나가 통째로 끊기지 않는 "단어"** 가 되고, 그 `min-content` 가
  `1fr`(= `minmax(auto,1fr)`) 열을 밀어 올린다. 2026-09-03 「지킨 다섯 가지」에서 ja 380px 카드가
  402px 로 부풀어 페이지에 가로 스크롤이 생겼다. → `minmax(0,1fr)` + `min-width: 0`.
  `overflow-wrap: break-word` 는 **해결책이 아니다**(min-content 계산을 바꾸지 않는다).
  기존 `.step h3/p` 에 keep-all 이 없는 것도 같은 이유로 보인다. 상세는 「지킨 다섯 가지」 절.
...
## 「지킨 다섯 가지」 섹션 — 타사와의 차이를 부정형으로 (2026-09-03)
...
### 형태는 **흰 카드 5장**이다 — 세로 목록으로 되돌리지 말 것
...
**05 는 전체 너비다**(`.made-item.wide` → `grid-column: 1 / -1`). 2열 그리드에서 다섯 번째가
한 칸만 차지하면 왼쪽에 홀로 남아 어색하고, 전체 너비로 두면 **개인정보라는 무게 있는 주제가
마지막에 강조**되는 효과도 겸한다.
...
- **01「매일 자동으로 안부」는 「지킨 다섯 가지」 01 및 히어로와 같은 말**이었다.
