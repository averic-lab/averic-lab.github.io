...
- **안전 코드 카드의 복사·공유 버튼과 점선 설명**: 좌우 위치는 `right`가 아니라 **논리 속성 `inset-inline-end`**로 잡는다 — 아랍어(RTL)에서 카드·버튼이 반대편으로 넘어갈 때 점선도 같이 따라가야 하기 때문이다. `right`로 쓰면 아랍어에서 점선만 엉뚱한 버튼을 가리킨다.
- 두 점선의 **길이를 어긋내 둔 것은 의도**다(복사 52px / 공유 18px). 버튼 간격이 36px뿐이라 라벨을 같은 높이에 두면 언어에 따라 겹친다. 길이를 맞추지 말 것.
- 점선이 카드 위로 뻗어야 하므로 `.codecard`는 `overflow: visible`이고, 원형 액센트만 안쪽 `.cbg`에서 자른다. 카드에 `overflow:hidden`을 되돌리면 점선이 잘린다.
