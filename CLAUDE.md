...
기존 「적당한 거리」(개인정보) 섹션을 대체했다. **설명 문장 없이 흐름만** 보여준다 — 폰 4대가 나란히 선다.

`① 대상자 홈(긴급 버튼)` → `② 확인 창` → `③ 보호자 알림(위치 보기)` → `④ 위치 지도`

- ⚠️ **한 폰에서 4장을 순환시키지 말 것**(초기 구현이 그랬고 되돌렸다, 2026-08-07). 순환은 지나간 단계를 지워버려 "1→2→3→4"를 동시에 보여주지 못한다. 각 단계가 **자기 폰**을 갖는 지금 구조가 요구사항이다.
- 강조가 2.4초씩 4단계를 돈다(9.6초 주기). `.fs1~.fs4`의 `--d`(지연)를 `.fstep`(들어올림)·`.fnum`(번호 점등)·`.fphone`(그림자) 세 애니메이션이 **공유**한다 — 지연을 개별 규칙에 흩뿌리지 말고 `--d`만 고칠 것.
- **크기 조절은 `.fstep`의 `--s`(scale)로만** 한다. `.fphone` 내부 좌표는 300×620 기준 실측값이라 내부 px 을 손대면 화면이 어긋난다. `.fstep`이 `calc(300px * var(--s))` 크기의 자리를 차지하고 `.fphone`이 그 안에서 축소된다.
- 클래스명에 **`f` 접두사가 붙은 이유**: 페이지에 이미 `.step`/`.step-num`(reveal 대상)이 있어 `.step`을 쓰면 그쪽 `opacity:0`을 물려받아 폰이 통째로 안 보인다. `.fstep`/`.fnum`/`.farrow`를 유지할 것.
- 화살표(`.farrow`)는 **한 줄일 때만** 쓴다. 2×2 격자에서는 방향이 어긋나므로 숨기고 번호가 순서를 맡는다. RTL은 flex 가 순서를 뒤집어 주므로 촉만 `scaleX(-1)`.
- 폭 구간: `>1080` 한 줄 `--s .66` / `~1080` 한 줄 `--s .5` / `~820` 2×2 `--s .62` / `~620` 2×2 `--s .5` / `~400` 2×2 `--s .42`. **820px 까지 한 줄을 유지**하는 건 축소가 격자로 접는 것보다 순서를 잘 전달하기 때문이다.
- 화면 문구는 전부 앱 번역 파일에서 온다(`APP_SOS_*` / `APP_MAP_*`). 확인 창 본문의 `\n`은 `build.py`가 `<br>`로 바꾼다.
- `prefers-reduced-motion`에서는 강조만 끈다. 4장이 이미 다 보이므로 **흐름은 그대로 전달된다**(순환 방식과 달리 손실이 없다).
