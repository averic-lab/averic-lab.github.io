...
기존 「적당한 거리」(개인정보) 섹션을 대체했다. **설명 문장 없이 흐름만** 보여준다 — 폰 화면 4장이 순환한다.

`① 대상자 홈(긴급 버튼)` → `② 확인 창` → `③ 보호자 알림(위치 보기)` → `④ 위치 지도`

- 전환은 **CSS 애니메이션만** 쓴다(JS 없음). 13.6초 주기 ÷ 4 = 한 장당 3.4초. `.fr1~.fr4`의 `animation-delay`와 `.fdots i`의 delay가 **같은 값**이어야 표시가 맞는다 — 한쪽만 바꾸지 말 것.
- 탭 표시(`.tap`)도 같은 3.4초 주기라 화면이 바뀌는 순간에 손끝이 닿는다.
- 화면 문구는 전부 앱 번역 파일에서 온다(`APP_SOS_*` / `APP_MAP_*`). 확인 창 본문의 `\n`은 `build.py`가 `<br>`로 바꾼다.
- `prefers-reduced-motion`에서는 순환을 멈추고 **첫 화면만** 남긴다 — 흐름은 못 보여도 화면 자체는 온전하다.
