...
공통 전제: **G+S(보호자가 대상자 겸함) 방식**으로 테스트하되, "G+S" 같은 내부 약어는 노출하지 않고 일반어("안부 보호 활성화", "나도 안부 보호 받기")로 표현한다. Android 전용. 버그 보고 메일: **l10s18bok@naver.com**.
...
- 안부 보호 활성화 버튼: **나도 안부 보호 받기** (활성화 후 같은 자리 → **내 안전 코드 확인**)
- 안부시각 변경: **안부 푸시 알림 시각 변경**
- 경고 해소 버튼: **안전확인 완료**
- 수동 보고: **지금 바로 안전 보고하기** / 긴급: **도움이 필요해요**
- ⚠️ 제공된 설정 스크린샷은 **활성화 후** 상태("내 안전 코드 확인")라, 테스터가 처음 보는 "나도 안부 보호 받기"와 다르다 — 두 페이지 모두 이 차이를 명시하고 있으니 유지할 것.
