...
- **지도는 미국 본토 실제 윤곽**이다(`.fusmap`). Natural Earth 110m 국경을 알버스 등적 원추도법(표준위선 29.5/45.5, 중앙자오선 -96)으로 투영한 좌표를 SVG path 로 박았다. 핀은 `<circle>`로 **SVG 안에** 둬서 지도와 같이 스케일된다 — 바깥에 두면 화면 크기마다 위치가 어긋난다. ⚠️ 윤곽을 기억으로 손그림하지 말 것(초기 시도에서 y축이 뒤집힌 채 통과할 뻔했다 — 반드시 렌더해서 **눈으로** 볼 것).
